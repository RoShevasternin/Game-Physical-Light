package com.rostislav.physical.light.game.actors.checkbox
...
open class ACheckBox(
    ...
    fun getStyleByType(type: Static.Type) = when(type) {
        Static.Type.TRUE_FALSE -> Static.ACheckBoxStyle(
            default = screen.game.assets.cbFalse,
            checked = screen.game.assets.cbTrue,
        )
        Static.Type.MUSIC -> Static.ACheckBoxStyle(
            default = screen.game.assets.music_def,
            checked = screen.game.assets.music_check,
        )
    }
